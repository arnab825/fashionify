...
function BrandLogo({ className = "w-8 h-8", textClassName = "text-2xl", showText = true, to = "/" }) {
  return (
    <Link to={to} className="flex items-center gap-2 hover:opacity-90 transition-opacity">
      <svg
        className={className}
        viewBox="0 0 200 200"
        fill="none"
      ...
      {showText && (
        <span className={`font-display font-black tracking-tighter uppercase ${textClassName}`}>
          Fashionify
        </span>
      )}
    </Link>
